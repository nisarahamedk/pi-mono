...
- Optional lifecycle control: set `hostBrowser.ensureRunning=true` and `hostBrowser.launchCommand` so mom auto-starts host Chrome CDP during gateway startup (and re-checks on later runs), not per cron run.
